...
const FormCadastro: React.FC<PropsNavigation> = ({
  receiveEntry,
  valor,
  setValor,
  tipoLancamento,
  ...
  const [descricao, setDescricao] = useState(
    receiveEntry?.descricaoLancamento ? receiveEntry?.descricaoLancamento : '',
  );
  const [dataPagamento, setDataPagamento] = useState(
    receiveEntry?.parcelasLancamento[0].dataParcela
  ...
  useEffect(() => {
    if (
      receiveEntry?.categoryLancamento &&
      typeof receiveEntry?.categoryLancamento != 'string'
    ) {
  ...
  const [selectedConta, setSelectedConta] = useState<Conta | null>(
    receiveEntry?.parcelasLancamento[0].contaParcela
      ? receiveEntry?.parcelasLancamento[0].contaParcela
      : null,
  );

  const [parcelas, setParcelas] = useState(
    receiveEntry?.parcelasLancamento
      ? String(receiveEntry.parcelasLancamento.length)
      : '1',
  );

  const [dataParcelas, setDataParcelas] = useState<Parcela[]>(
    receiveEntry?.parcelasLancamento
      ? receiveEntry.parcelasLancamento
      : [
          {
  ...
  const changeParcela = (
    ...
    if (num < newDataParcelas.length) {
      for (var i = 0; i < num; i++) {
        ...
        newDataParcelas[i].dataParcela =
          i == 0 ? new Date(date) : addMonths(new Date(date), 1);
      ...
      for (var i = 0; i < num; i++) {
        const adicaoDeUmMes =
          i == 0
            ? new Date(date)
            : addMonths(new Date(aux[i - 1].dataParcela), 1);
        ...
        if (i < newDataParcelas.length) {
          ...
          aux.push({
            ...
            valorParcela: isNaN(valorParcelaDividido)
              ? 0
              : valorParcelaDividido,
            statusParcela: status,
            lancamentoParcela: -1,
            indexOfLancamento: i,
          });
  ...
  return (
    <ContainerForm>
      ...
      <ContainerDetalhes style={{ display: detalhes ? 'flex' : 'none' }}>
        ...
        <DetalhesMensal style={{ display: mensal ? 'flex' : 'none' }}>
          Você {tipoLancamento == 'despesa' ? 'pagará' : 'receberá'}{' '}
          {valor == ''
            ? 'R$ 0,00'
          ...
          mensalmente a partir de {dataPagamento.toLocaleDateString()}, os
          valores poderão ser trocados no futuro
        </DetalhesMensal>

        <InputControl style={{ display: mensal ? 'none' : 'flex' }}>
          <InputText
            onClear={() => {}}
            showClearIcon={false}
        ...
        <SectionCardsParcelas style={{ display: mensal ? 'none' : 'flex' }}>
          {
            <FlatList
              data={dataParcelas}
              renderItem={({ item }) => (
                <ItemParcela
                  ...
                  setDataParcelas={setDataParcelas}
                  tipoLancamento={tipoLancamento}
                />
              )}
              horizontal
