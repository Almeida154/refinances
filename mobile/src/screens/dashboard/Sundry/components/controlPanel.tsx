...
const ControlPanel = () => {
    ...
    function handleSubmit(necessidades, produtos, verba) {
        ...
        for (var i = 1; i < m; i++) {
            ...
            for (var l = 0; l < produtos[i - 1].conectaNecessidades.length; l++) {
                let soma = produtos[i - 1].conectaNecessidades[l].valor + necessidades[produtos[i - 1].conectaNecessidades[l].index - 1].quantidade
                arranjoDasNecessidades[i] += soma > 100 ? produtos[i - 1].conectaNecessidades[l].valor - (soma - 100) : produtos[i - 1].conectaNecessidades[l].valor
            }

            for (var j = produtos[i - 1].preco; j <= verba; j++) {
                let primeiro = pd[i - 1][j]
                let segundo = pd[i - 1][j - produtos[i - 1].preco] + arranjoDasNecessidades[i]
    ...
    return (
        <SafeAreaView style={styles.container}>
            ...
            />
            <Button title="Otimizar" onPress={() => handleSubmit(wants, items, verba)} />
